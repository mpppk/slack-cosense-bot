...
Slack チャンネルの description に `cosense:` 行で対象プロジェクトを書く。
複数登録でき、URL 全体ではなくプロジェクト名だけでよい。
...
- プレフィックス `cosense:` (大文字小文字不問、全角 `：`・`=` も可) の有無で
  自然文の説明と見分ける。プレフィックスの無い行は紐づけとして読まない
- 区切りはカンマ・空白・`、` のいずれも可
- 各トークンはプロジェクト名 (`niki-auth`) でも URL
  (`https://scrapbox.io/niki-auth`) でもよい
- 旧書式 (プレフィックス無しの URL / 素のプロジェクト名) も後方互換として読む

検索系ツールは紐づく全プロジェクトを横断し、ページ指定系・書き込み系は
`project` パラメータで1つに絞る (紐づけが1つだけなら省略可)
